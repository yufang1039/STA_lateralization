...
for i=1:3
    ...
    title([num2str(mydata.pos(d(i,:)))])
%     title([num2str(mydata.pos(d(i,:))) ': ' num2str(val,'%.2f')])
end

intensity = mydata.img(mydata.pos(1),mydata.pos(2),mydata.pos(3));
if ~isnan(intensity)
    dataTemp = mydata.img(~isnan(mydata.img(:)));
    focality = (sum(dataTemp(:) >= intensity*0.5))^(1/3)/10;
else
    focality = nan;
end
if ~isempty(mydata.mri2mni)
    ...
    coordInfo = {['Voxel: ' num2str(mydata.pos(1)) ',' num2str(mydata.pos(2)) ',' num2str(mydata.pos(3))],...
        ...
        ['Modul. Depth: ' num2str(intensity,'%.2f') ' V/m'], ...
        ['Focality: ' num2str(focality,'%.2f') ' cm']};
